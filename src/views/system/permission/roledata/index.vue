<template>
  <div>
    <DynamicTable
      row-key="id"
      header-title="角色管理"
      :data-request="getRoledataListByPage"
      :columns="columns"
      bordered
      size="small"
    >
...
<script lang="ts" setup>
  ...
  import {
    getRoledataListByPage,
    updateRoledata,
    createRoledata,
    deleteRoledata,
    getRoledataInfo,
  } from '@/api/system/roledata';
  import { getDeptDataList } from '@/api/system/deptdata';
  ...
  const openMenuModal = async (record: Partial<TableListItem>) => {
    const [formRef] = await showModal<API.UpdateRoleParams & API.CaptchaParams>({
      modalProps: {
        ...
        onFinish: async (values) => {
          ...
          await (record.id ? updateRoledata : createRoledata)(params);
    ...
    const [deptData, menuData] = await Promise.all([getDeptDataList(), getMenuList()]);
    ...
    // 如果是编辑的话，需要获取角色详情
    if (record.id) {
      const data = await getRoledataInfo({ roleId: record.id });
  ...
  const delRowConfirm = async (record: TableListItem) => {
    await deleteRoledata({ roleIds: [record.id] });
    dynamicTableInstance?.reload();
  };
